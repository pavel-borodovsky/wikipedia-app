...
class WikipediaController extends Controller
{
    public function importArticles(Request $request)
    {
        ...
        if (isset($data['query']['search'])) {
            ...
            $articleResponse = Http::get('https://ru.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&explaintext=true&titles=' . $articleTitle)->json();
